...
class Graph():
    ...
    def add_edge(self, src, dst):
        if not src in self.edge_list:
            self.add_node(src)
        if not dst in self.edge_list:
            self.add_node(dst)
    ...
    def print_communities(self, filename):
        ...
        print (f'${num_comms} different communities identified and written to ${filename}')
...
class SLPA():
    ...
    def propagate(self, random_nodes):
        for random_node in random_nodes:
            ...
            if not most_popular_message == -1:
                if not most_popular_message in listener.memory:
    ...
    def post_process(self):
        nodes_to_remove = []  # Create a list to store nodes to be removed from memory
    
        for node in self.graph.node_list.values():
            keys_to_remove = []  # Create a list to store keys to be removed from memory
            for key in node.memory.keys():
                probability = float(node.memory[key]) / float(node.memory_count)
                if probability < self.threshold:
                    keys_to_remove.append(key)  # Add keys to be removed to the list
            for key in keys_to_remove:
                del node.memory[key]  # Remove keys from memory
